...
export const SalesForm = () => {
    ...
    const handleUpdate = async (e: FormEvent) => {
        ...
        try {
            ...
            while (initialDownloadDate <= finalDate) {
                ...
                const accion = { description: `Actualizando ventas de ${format(initialDownloadDate, "MMMM", { locale: es })} ${getYear(initialDownloadDate)}`, updating: true, done: false }
                actions.push(accion)
                setActions(actions)

                const { data } = await api.put<ApiResponse<Sale[]>>(`/sales?domain=${domain}&initialDate=${format(initialDownloadDate, "yyyy-MM-dd")}&finalDate=${format(finalDownloadDate, "yyyy-MM-dd")}`)
    ...
    return <section>
        <div className="flex items-center justify-between mb-6">
            <h3 className="font-semibold text-lg">Actualizar ventas</h3>
        ...
        <div className="flex gap-4 w-full">
            ...
            {actions.length > 0 && <div className="flex flex-col gap-4 border rounded-md p-8 shadow-md bg-gray-100 w-fit min-w-96 h-fit">
                <h5 className="font-medium">Actualizando ventas</h5>
